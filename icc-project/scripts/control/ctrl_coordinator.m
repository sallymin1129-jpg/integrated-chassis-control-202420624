...
actuatorCmd.steerAngle = ...
    max(-LIM.MAX_STEER_ANGLE,...
    min(LIM.MAX_STEER_ANGLE,...
    latCmd.steerAngle));
...
if lonCmd.Fx_total < 0
    ...
    FxBrake = abs(lonCmd.Fx_total);

    T_front = 0.6 * FxBrake * VEH.rw / 2;
    T_rear  = 0.4 * FxBrake * VEH.rw / 2;

    brakeTorque(1:2) = T_front;
    brakeTorque(3:4) = T_rear;
end

Mz = latCmd.yawMoment;

dTf = Mz * 0.6 / VEH.track_f;
dTr = Mz * 0.4 / VEH.track_r;
