...
ctrlState.intError = max(-CTRL.LON.intMax, ...
                     min(CTRL.LON.intMax, ctrlState.intError));

Fx = CTRL.LON.Kp*error + CTRL.LON.Ki*ctrlState.intError;


maxForceStep = LIM.MAX_JERK * 1500 * dt;
Fx = max(ctrlState.prevForce - maxForceStep, ...
     min(ctrlState.prevForce + maxForceStep, Fx));


if isfield(ctrlState, 'wheelSlip') && any(ctrlState.wheelSlip ~= 0)
    slipExceeded = any(abs(ctrlState.wheelSlip) > 0.12);
else
    
    slipExceeded = (ax < -4.0);
end

if slipExceeded && Fx < 0
    Fx = Fx * 0.6; 
   
    ctrlState.intError = ctrlState.intError * 0.8;
end

ctrlState.prevForce = Fx;
forceCmd.Fx_total = Fx;
forceCmd.brakeRatio = double(Fx < 0);
end
