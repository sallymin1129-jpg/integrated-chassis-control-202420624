...
ctrlState.intError = max(-CTRL.LAT.intMax,...
                         min(CTRL.LAT.intMax,...
                         ctrlState.intError));

dError = (error - ctrlState.prevError)/dt;

gainScale = min(vx/40,1.2);

steerCmd = gainScale * ...
          (CTRL.LAT.Kp*error + ...
           CTRL.LAT.Ki*ctrlState.intError + ...
           CTRL.LAT.Kd*dError);

ctrlState.prevError = error;

deltaAdd.steerAngle = max(-LIM.MAX_STEER_ANGLE,...
...
if abs(slipAngle) > beta_th
    deltaAdd.yawMoment = ...
        -1000 * sign(slipAngle) * ...
        (abs(slipAngle)-beta_th) * gainScale;
else
